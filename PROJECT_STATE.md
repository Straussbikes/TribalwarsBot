...
**Estado Geral:** Fase 2 em Progresso (Edifício Principal, Praça, Micro-Farming e Recrutamento Concluídos)  
...
| **Fase 2** | **Módulos de Ações (`game.php`)** | 🔄 Em Curso | `main` (Edifício Principal), `place` (Praça de Reunião), `farm` (Micro-Farming) e `recruitment` (Quartel/Estábulo/Oficina) concluídos. Total de 44 testes unitários. Próximos: `snob` (cunhagem) e `scavenge`. |
...
│
├── tests/                           # Suíte de testes unitários automatizados (44 testes, 100% OK)
│   ├── __init__.py
