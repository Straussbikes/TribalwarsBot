...
| **Fase 16** | **Otimização de Farm, Resiliência de Agendamento & Prevenção de Concorrência** | ✅ Concluída | Eliminação do bug mutuamente exclusivo de cooldown/sobreposição em `farm.py` (verificação cumulativa de `target_cooldown_minutes` e `avoid_concurrent_attacks`); proteção de bárbaras de mapa via `_recent_farm_targets`; blindagem contra duplicação de tarefas no `TaskScheduler` (`has_task`) e cancelamento de tarefas órfãs; persistência de parâmetros de farm no Cloud SQL (`game_worlds.config_data`) e UI cockpit; 2 novas suites de testes unitários (`test_farm_cooldown_and_concurrency.py`, `test_emergency_farm_priority.py`). |
